...
class Parser(object):
    ...
    def _get_value(self, ws_int, signed=False):
        # ws_int is expected to be a string of space/tab characters
        # For signed integers the first char is the sign bit and the
        # remainder are the magnitude bits. It's possible the magnitude
        # is empty (e.g. sign followed immediately by LF), so handle
        # that case and treat it as zero instead of letting int('', 2)
        # raise a ValueError.
        if ws_int is None:
            return 0
        sign = ""
        if signed:
            sign = "-" if NUM_SIGN_CONST.get(ws_int[0]) == "NEGATIVE" else ""
            ws_int = ws_int[1:]
        # Map whitespace chars to binary digits, ignoring any unknown
        # characters defensively by mapping them to empty string.
        bits = "".join([NUM_CONST.get(i, "") for i in ws_int])
        number = int(bits, 2) if bits != "" else 0
    ...
    def parse(self):
        while self.instruction_ptr < self.num_of_tokens:
            ...
            if token[1].type in HAS_ARGS:
                signed = True if token[1].type == "PUSH" else False
